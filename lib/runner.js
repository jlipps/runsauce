...
exports.runTest = o_O(function*(multiRun, test, driver, caps, opts, onSauce) {
  ...
  log(" - Session finished in " + (result.time / 1000) + "s" + " (startup " +
      "time: " + (result.startupTime / 1000) + "s, " +
      (result.startupTime / result.time * 100) + ")");
  return result;
});
...
exports.run = o_O(function*(opts, caps) {
  ...
  console.log("Running " + testStr + " against " + opts.configName + " with caps:");
  ...
  if (multiRun) {
    ...
    var startSum = _.reduce(_.pluck(results, 'startupTime'), function(m, n) { return m + n; }, 0);
    var startAvg = startSum / results.length;
    console.log("Average test run time: " + (avg / 1000) + "s");
    console.log("Average test startup time: " + (startAvg / 1000) + "s (" +
                (startAvg / avg * 100) + "% of total)");
