...
async function runTestSet (testSpecs, opts, log, statusFn) {
  ...
  checkRuns();
  await allDone.promise; // wait till we're done with all the tests
  return results;
}
...
async function run (opts, log = true, statusFn = null) {
  ...
  if (!statusFn) {
    if (numTests > 1) {
      ...
      // if we're not running more than one test, we don't care about the
      // status since everything will be printed out after the test has
      // finished anyway. So pipe it to /dev/null :-)
      statusFn = () => {};
    }
  }
